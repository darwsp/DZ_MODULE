...
const double YAK40_MASS = 13900.0;          // Масса самолета, кг
const double YAK40_WING_AREA = 70.0;       // Площадь крыла, м²
const double YAK40_NOMINAL_THRUST = 3 * 147100.0; // Суммарная тяга, Н
const double YAK40_CY0 = 0.2;               // Коэффициент подъемной силы при нулевом угле атаки
const double MAX_THRUST_PERCENT = 0.9;     // Реалистичный процент тяги
const double INITIAL_ALTITUDE = 400.0;      // Начальная высота, м
const double FINAL_ALTITUDE = 6100.0;       // Конечная высота, м
const double INITIAL_VELOCITY = 260.0 / 3.6; // Начальная скорость, м/с
const double FINAL_VELOCITY = 860.0 / 3.6;  // Конечная скорость, м/с
const double GRAVITY = 9.81;               // Ускорение свободного падения, м/с²
const double PI = 3.14159;
const double Cy0 = 0.2;
...
const double MAX_VERTICAL_SPEED = 10.0; // Максимальная скорость набора высоты
...
public:
    ...
    void plotTrajectory() const {
        ...
        fprintf(gp, "set multiplot layout 2,2 title 'parametres of flight of yak-40'\n");

        // Высота от времени
        fprintf(gp, "set title 'height from time'\n");
        fprintf(gp, "set xlabel 'time, c'\n");
...
public:
    ...
    Aircraft(double m = YAK40_MASS, double wing = YAK40_WING_AREA)
        : wing_area(wing), initial_mass(m), env(), mass(m) {

        thrust = YAK40_NOMINAL_THRUST * MAX_THRUST_PERCENT;
    ...
    double calculate_alpha(double current_altitude, double current_velocity) {
        ...
        double alpha = (mass * GRAVITY - q * YAK40_WING_AREA * YAK40_CY0) / (q * YAK40_WING_AREA * grad_Cy + current_P);
        return alpha;

    }
    // Расчет градиента коэффициента подъема
    double grad_Cy_alpha() const{
        double b = 25; // размах крыла
        double AR = b*b/YAK40_WING_AREA;
...
double getLiftCoefficient(double alpha) const{
    ...
    // Для расчета воспользуемся линейной зависимостью
        double grad_Cy = grad_Cy_alpha();
        double Cy = YAK40_CY0 + grad_Cy * alpha;
        if (Cy > 1.1)
            return 1.1;
        return Cy;
}
    ...
    double computeLiftForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double L = 0.5 * rho * current_velocity * current_velocity * YAK40_WING_AREA * Cy;
        return L;
    }
    ...
    double computeDragForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double X = 0.5 * rho * current_velocity * current_velocity * YAK40_WING_AREA * Cx;
        return X;
    }
    // Расхода топлива
    ...
    double total_thrust(double current_altitude){
        ...
        return YAK40_NOMINAL_THRUST*MAX_THRUST_PERCENT * pow(current_p/p_0, 0.7); // коррекция тяги по плотности и давлению
    }
};
...
public:
    ...
    double calculate_razgon(double altitude, double initial_velocity, double final_velocity, Aircraft& ac){
        ...
        double a_x = ((current_P * cos(alpha_rad)) - q * Cx * YAK40_WING_AREA) / ac.mass;
...
int main() {
    ...
    try {
        ...
        Trajectory trajectory = solver.computeOptimalTrajectory(yak40);
        trajectory.saveToCSV("yak40_trajectory_realistic.csv");
